...
pub trait TeaqlRepositoryProvider: TeaqlRuntime {
    ...
    type WorkItemRepository<'a>: TeaqlEntityRepository + 'a
    where
        Self: 'a;

    fn work_item_repository(&self) -> Result<Self::WorkItemRepository<'_>, ContextError>;
}
...
impl TeaqlRepositoryProvider for teaql_runtime::UserContext {
    ...
    type WorkItemRepository<'a> = teaql_runtime::EntityDataService<'a, crate::runtime::DataServiceExecutor>
    where
        Self: 'a;

    fn work_item_repository(&self) -> Result<Self::WorkItemRepository<'_>, ContextError> {
        self.entity_data_service::<crate::runtime::DataServiceExecutor>("WorkItem")
    }
}
