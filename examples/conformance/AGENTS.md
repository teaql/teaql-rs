...
```bash
cargo teaql --input models/runtime-example-conformance-service.xml rust-assist-[action]/[entity-name]
```

> `models/runtime-example-conformance-service.xml` is the default model path. If the model file is located elsewhere, adjust the `--input` path to match the actual file location in this project.
