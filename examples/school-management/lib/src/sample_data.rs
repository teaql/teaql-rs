    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
    ...
    context.user_context().transaction_data(|| async {
        Box::pin(generate_schools(context, &mut state)).await.map_err(|e| {
...
where
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
    let list = Q::school_types().comment("what: inspect existing entities before sample-data initialization").purpose("why: avoid duplicate sample records").execute_for_list(context).await.unwrap_or_default();
    for item in list {
    ...
    C: TeaqlRuntime + ?Sized + crate::TeaqlRepositoryProvider,
{
        if state.ids("Platform").is_empty() {
            state.record_skipped(crate::School::ENTITY_NAME, "Required dependency Platform is missing in reference pool".to_string());
        ...
        if state.ids("School Type").is_empty() {
            state.record_skipped(crate::School::ENTITY_NAME, "Required dependency School Type is missing in reference pool".to_string());
    ...
    for i in 0..fanout {
        let mut entity = Q::schools().comment("what: initialize a sample entity").purpose("why: populate the requested sample dataset").new_entity(context);
