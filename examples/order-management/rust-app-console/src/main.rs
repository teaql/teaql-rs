...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ctx = service_runtime(ServiceRuntimeConfig {
    ...
    let platform_id = if let Some(platform) = platforms.first() {
        ...
        let request = Q::commerce_platforms()
            ...
            .purpose(PURPOSE);
        let mut platform = request.new_entity(&ctx);
        platform.update_name("Northwind Demo");
        let platform_id = platform.id();
        platform.audit_as("Create quick-start commerce platform").save(&ctx).await?;
    ...
    if seeded_orders.is_empty() {
        ...
        order.update_order_number("WEB-2026-001")
            ...
            .update_commerce_platform_id(platform_id);
        order.audit_as("Create deterministic quick-start order").save(&ctx).await?;
    ...
    if let Some(preset) = existing {
        ...
        let mut preset = request.new_entity(&ctx);
        ...
        preset.audit_as("Save idempotent quick-start search preset").save(&ctx).await?;
